...
def get_sizes(bag, topics=None, index=0, scale=1.0, start_time=rospy.Time(0), stop_time=rospy.Time(sys.maxsize)):
    logging.debug("Resizing height to topic %s (index %d)." %
                  (topics[index], index))
    sizes = []
    ...
    for topic in topics:
        ...
        try:
            iterator = bag.read_messages(
                topics=topic, start_time=start_time, end_time=stop_time)  # , raw=True)
            msg = next(iterator)[1]  # read one message
            sizes.append((msg.width, msg.height))
        except:
            pass
            logging.critical(
                "No messages found for topic %s, or message does not have height/width. Make sure a '/' is added before the topic" % topic)
            traceback.print_exc()
            sys.exit(1)
    ...
    for i in range(len(topics)):
        logging.info('Topic %s originally of height %s and width %s' %
                     (topics[i], sizes[i][0], sizes[i][1]))
        image_height = sizes[i][1]
        image_width = sizes[i][0]

        # rescale to desired height while keeping aspect ratio
        sizes[i] = (int(1.0*image_width*target_height /
                    image_height), target_height)
        logging.info('Topic %s rescaled to height %s and width %s.' %
                     (topics[i], sizes[i][0], sizes[i][1]))
...
def write_frames(bag, outdir, topics, sizes, start_time=rospy.Time(0),
    ...
    images = [np.zeros((sizes[i][1], sizes[i][0], 3), np.uint8)
              for i in range(len(topics))]
    iterator = bag.read_messages(
    ...
    for topic, msg, t in iterator:
        ...
        cv2.imwrite(str(outpath), merged_image)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'bagfile_path', help='Specifies the location of the bag file. Can set directory to glob all bag files, or file to choose one bag')
    parser.add_argument('topics', nargs='+',
                        help='Image topics to merge in output video.')
    ...
    global_count = 0
    if args.bagfile_path[-4:] == '.bag':
        bagfiles = [args.bagfile_path]
    else:
        if args.bagfile_path[-1] != '/':
            args.bagfile_path += '/'
        bagfiles = sorted(glob.glob(args.bagfile_path + "*.bag"))
    for bagfile in bagfiles:
        logging.info('Proccessing bag %s.' % bagfile)
        bag = rosbag.Bag(bagfile, 'r')

        logging.info('Calculating video sizes.')
        sizes = get_sizes(bag, topics=args.topics, index=args.index,
                          scale=args.scale, start_time=start_time, stop_time=stop_time)

        logging.info('Calculating final image size.')
        out_width, out_height = calc_out_size(sizes)
        logging.info('Resulting video of width %s and height %s.' %
                     (out_width, out_height))

        count = write_frames(bag=bag, outdir=args.outdir, topics=args.topics, sizes=sizes,
                             start_time=start_time, stop_time=stop_time,
                             encoding=args.encoding,
                             skip=args.skip, count=global_count)
